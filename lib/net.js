...
// 問題是、karpathy 加完後才用 relu，我是直接在 ConnectLayer 中用 sigmoid。
class Rnn extends G.CGate {
  constructor(inputs, hSize, out, net) {
    ...
    let ho = net.newVector(hSize)
    let hidden = net.newVector(hSize)
    let s = net.newVector(out.length)
    this.gates = [
      new ConnectLayer(inputs, hi, net, -0.2, 0.2), // 問題是、karpathy 加完後才用 relu，我是直接用 sigmoid。
      new ConnectLayer(hidden, hh, net, -0.2, 0.2), // 循環層
      new Layer([hi, hh], hidden, G.Add),
      /*
      new ConnectLayer(inputs, hi, net, -0.2, 0.2, null),
      new ConnectLayer(hidden, hh, net, -0.2, 0.2, null),
      new Layer([hi, hh], ho, G.Add),
      new Layer([ho], hidden, G.Sigmoid),
      */
