...
/*
function updateGradX(x, y, out, df) {
  x.grad += df(x.value, y.value) * out.grad
  ...
  if (typeof x.value === 'number') {
    ...
    let len = x.length
    for (let i = 0; i < len; i++) {
      x[i].grad += df(x[i].value, y[i].value) * out[i].grad
    }
  }
}

function updateGradY(x, y, out, dfy) {
  y.grad += dfy(x.value, y.value) * out.grad
  if (typeof x.value === 'number') {
    y.grad += dfy(x.value, y.value) * out.grad
  } else {
    let len = x.length
    for (let i = 0; i < len; i++) {
      y[i].grad += dfy(x[i].value, y[i].value) * out[i].grad
    }
  }
}
*/