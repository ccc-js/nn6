...
class Mlp extends N.NetGate {
  constructor(inputs, hSizes, out, net) {
    super(inputs, out, net)
    let hLen = hSizes.length
    let hiddens = [ inputs ]
    for (let i=0; i<hLen; i++) {
      hiddens[i+1] = net.newVector(hSizes[i])
      this.gates.push(new N.Connect([ hiddens[i] ], hiddens[i+1], net, -0.2, 0.2))
    }
    this.gates.push(new N.Connect([ hiddens[hLen] ], out, net, -2.0, 2.0))
  }
}
